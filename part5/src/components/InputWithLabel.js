...
const CustomInputField = ({ type, name, state, eventHandler, step = 1, pattern, required, max, min }) => {
  if (type === 'textarea')
...
const InputWithLabel = ({ type, name, label, state, setState, step, integer, pattern, required, max, min }) => {
  ...
  return (
    <div>
      ...
      <CustomInputField type={type} name={name} state={state} eventHandler={eventHandler} step={step} pattern={pattern} required={required} max={max} min={min} />
      <br />
    </div>
  );
};
